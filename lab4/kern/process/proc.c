...
// 调用kernel_thread函数时，需要指定：
// fn：线程的执行入口(例如：init_main)，
// arg：入口函数的参数(例如："Hello world!")，
// clone_flags：指定是否需要采取写时复制的机制进行fork时父子进程的内存映射。
int
kernel_thread(int (*fn)(void *), void *arg, uint32_t clone_flags) {
    struct trapframe tf;
    // 构建一个临时的中断栈帧tf，用于do_fork中的copy_thread函数
    // (因为线程的创建和切换是需要利用CPU中断返回机制的)
    ...
    return do_fork(clone_flags | CLONE_VM, 0, &tf);//返回子进程的id号
}
...
copy_thread(struct proc_struct *proc, uintptr_t esp, struct trapframe *tf) {
    ...
    // 将参数tf中的结构体数据复制填入上述proc->tf指向的位置
    // (正好是上面struct trapframe指针-1腾出来的那部分空间)
...
//设置好执行现场后，一旦uCore调度器选择了initproc执行，就需要根据initproc->context中保存的执行现场来恢复initproc的执行。
...
//由于initproc的中断帧占用了实际给initproc分配的栈空间的顶部，所以initproc就只能把栈顶指针context.esp设置在initproc的中断帧的起始位置。
//根据context.eip的赋值，可以知道initproc实际开始执行的地方在forkret函数（主要完成do_fork函数返回的处理工作）
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    // 由于是fork，因此fork的一瞬间父子线程的内存空间是一致的
...
proc_init(void) {
    ...
    set_proc_name(idleproc, "idle"); // 设置idle线程的名称
    nr_process ++;
