...
check_swap(void)
{
    //backup mem env
     ...
     struct vma_struct *vma = vma_create(BEING_CHECK_VALID_VADDR, CHECK_VALID_VADDR, VM_WRITE | VM_READ);//4KB——24KB
     assert(vma != NULL);
     ...
     // 由于之前没有建立页表，所以会产生page fault异常，
     // 如果完成练习1，则这些从4KB~20KB的4虚拟页会与ucore保存的4个物理页帧建立映射关系；
     pgfault_num=0;
