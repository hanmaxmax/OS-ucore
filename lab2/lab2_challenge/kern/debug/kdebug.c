...
print_stackframe(void) {
    ...
    for(i = 0; i<STACKFRAME_DEPTH&&ebp; i++){
        ...
        eip = ((uint32_t *)ebp)[1];//更新ebp和eip  (uint32_t *)ebp表示ebp中内容指向的内存地址，即母函数的ebp值 神奇之处！！
        ebp = ((uint32_t *)ebp)[0];
   }
   /*给ebp赋值之前，原ebp值已经被压栈（位于栈顶），而新的ebp又恰恰指向栈顶。此
   时ebp寄存器就已经处于一个非常重要的地位，该寄存器中存储着栈中的一个地址（原
   ebp入栈后的栈顶），从该地址为基准，向上（栈底方向）能获取返回地址、参数值，向
   下（栈顶方向）能获取函数局部变量值，而该地址处又存储着上一层函数调用时的ebp值。*/
}
