...
#define nr_free (free_area.nr_free) //链表里 总的空闲页数？
...
/*default_init_memmap()
* 传入物理页基地址，和物理页的个数(个数必须大于0)，然后对每一块物理页进行设置：
* 先判断是否为保留页，如果不是，则进行下一步。
* 将标志位清0，连续空页个数清0，然后将标志位设置为1，将引用此物理页的虚拟页的个数清0。然后再加入空闲链表。
* 最后计算空闲页的个数，修改物理基地址页的property的个数为n。
*/
static void
default_init_memmap(struct Page *base, size_t n) {
    ...
    // 指导书：这里需要注意的是用到此成员变量的这个Page比较特殊，是这个连
    // 续内存空闲块地址最小的一页（即头一页， Head Page）。
    base->property = n; //修改base的连续空页值为n 
    SetPageProperty(base);
    nr_free += n; //计算空闲页总数
    list_add(&free_list, &(base->page_link)); // base加入空闲链表 只加一个entry 对这样才说得通
}
/* default_alloc_pages()
* 这个函数是用来分配空闲页的。
* 首先判断空闲页的大小是否大于所需的页块大小。
* 如果小于空闲页的大小。则遍历整个空闲链表，寻找合适的空闲页块。
* 如果找到的空闲页块的大小大于所需大小。则分割页块：将*（base+n）作为新的页首。
* 如果合适则不进行操作，
...
default_alloc_pages(size_t n) {
    ...
    // list_entry_t *le,*len = &free_list; //空闲链表的头部和长度
    ...
    //             //?理解? 只有head设置了property，双向链表还是以页为单位链起来的
    ...
    while ((le = list_next(le)) != &free_list) { //遍历整个空闲链表
        struct Page *p = le2page(le, page_link); //转换页结构
        if (p->property >= n) {
            page = p;
            break;
        }
    ...
    if (page != NULL) {
        if (page->property > n) {
            struct Page *p = page + n;
            p->property = page->property - n;
            SetPageProperty(p); //为新的首部设置property
            list_add_after(&(page->page_link), &(p->page_link)); //把新的页块p插入freelist
        }
        list_del(&(page->page_link));//将page从free_list中清除
        nr_free -= n; //更新nr_free的值
        ClearPageProperty(page); //清楚page的property位
    }
    return page;
}
...
* 在freelist中查找合适的位置以供插入。
* 改变被释放页的标志位，以及头部的计数器尝试在freelist中向高地址或低地址合并。
*/
static void
default_free_pages(struct Page *base, size_t n) {
    ...
    // list_entry_t *le = &free_list; 
    // struct Page * p;
    // while((le=list_next(le)) != &free_list) {    //寻找合适的位置（为了按地址顺序插入）
    //   p = le2page(le, page_link); //获取链表对应的Page
    //   if(p>base){ 
    //     //base在p（le entry标识）前插入   
    //     //le已经被更新了！！
    //     break;
    //   }
    // }
    ...
    for (; p != base + n; p ++) {
        ...
        assert(!PageReserved(p) && !PageProperty(p));
        p->flags = 0;
        set_page_ref(p, 0); //ref：映射到此物理页的虚拟页个数
    }
    base->property = n;
    SetPageProperty(base);
    list_entry_t *le = list_next(&free_list);
